...
export default function ClientPage({ slug, initialData }: Props){
    const {
        allResources,
        resources,
        setOneMda,
    ...
    //     enabled: !!slug,
    // });
    // const mda = loadedData?.data.data ?? initialData;
    ...
    // });
    // console.log("🚀 ~ Mdas ~ mda:", mda?.resources);
    ...
    return (
        <>
            <Head>
                <title>
                    {mda?.mda?.name
                        ? `${mda?.mda?.name} - Plateau State Government`
                        : `${mda?.mda?.abbreviation || "MDA"
                        } - Plateau State Government`}
                </title>
                <meta
                    name="description"
                    content={
                        mda?.mda?.hero?.description ||
                        `Official page for ${mda?.mda?.name ||
                        mda?.mda?.abbreviation ||
                        "this MDA"
                ...
                <meta
                    name="keywords"
                    content={`${mda?.mda?.name}, Plateau State, Government, MDA, ${mda?.mda?.name} services`}
                />
                ...
                <script type="application/ld+json">
                    {JSON.stringify({
                        ...
                        name: mda?.mda?.name,
                        description: mda?.mda?.hero?.description,
                ...
                <meta
                    property="og:title"
                    content={
                        mda?.mda?.name || mda?.mda?.abbreviation
                            ? `${mda?.mda?.name ||
                            mda?.mda?.abbreviation
                            } - Plateau State Government`
                            : `${mda?.mda?.abbreviation || "MDA"
                            ...
                            <div className="w-full flex flex-col items-between justify-between gap-20 h-[70%]">
                                ...
                                <div className="w-full flex flex-col gap-8 lg:w-[65%]">
                                    ...
                                    <span className="w-full font-normal text-[14px] text-brand-main font-geistsans md:text-base lg:w-[75%]">
                                        Find key information quickly on a variety of important
                                        topics right here on the Plateau Government website.
                                    </span>
                                </div>
                                        ...
                                        <>
                                            <div className="w-full flex items-center justify-center lg:justify-end">
                                                <div className="flex items-center gap-4">
                                                    {/* left arrow */}
                                                    <div
                                                        className={`cursor-pointer flex items-center ${activeNewsPage === 1
                                                    ...
                                                    {/* right arrow */}
                                                    <div
                                                        className={`cursor-pointer flex items-center ${activeNewsPage === totalNewsPages
                                                                ? "opacity-50 cursor-not-allowed"
                                                                : ""
                                                            }`}
                                            ...
                                            {/* <PaginationComponent
                      lightMode={true}
                      // totalNewsPages={mda?.pagination?.totalNewsPages}
